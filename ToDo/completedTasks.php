<?php
    ...
    $query = "SELECT id, title, category, date, status FROM list WHERE user_id = ? AND status = ?";
    $pst = $con->prepare($query);
    $pst->execute([$_SESSION["user_id"],"Completed"]);
    $lists = $pst->fetchAll();
        ...
        <ul class="navbar-nav me-auto mb-2 mb-lg-0">
          ...
          <li class="nav-item dropdown">
            ...
            <ul class="dropdown-menu" aria-labelledby="navbarDropdown">
              ...
              <li><a class="dropdown-item text-danger" href="completedTasks.php">Completed Tasks</a></li>
            ...
            <tr>
              <?php 
                if(isset($_GET["search"])){
                  ...
                  if(!empty($title)){
                    $showAll = false;
                    $query = "SELECT * FROM list WHERE title LIKE ? AND user_id= ? AND status = ?";
                    $pst = $con->prepare($query);
                    $val = "%". $title . "%";
                    $pst->execute([$val,$user_id,"Completed"]);
